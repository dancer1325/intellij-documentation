[//]: # (Source: https://www.jetbrains.com/help/idea/run-debug-configuration-kotlin-script.html)
[//]: # (Downloaded: 2025-12-17 19:58:22)

## Configuration tab

Item| Description  
---|---  
Script file| Specify here the path to a .kts file to be launched.  
...
Redirect input from| Specify a file to read the input from. Relative paths are prepended with the Working directory path.You can use [macros](built-in-macros.html) and [path variables](absolute-path-variables.html) in this field.For example, you can use the `$FilePrompt$` macro to prompt you for a file every time you launch this run configuration.  

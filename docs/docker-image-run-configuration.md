[//]: # (Source: https://www.jetbrains.com/help/idea/docker-image-run-configuration.html)
[//]: # (Downloaded: 2025-12-17 19:25:08)

# Docker Image run configuration

Use this type of configuration to run a Docker container from a locally existing image that you either [pulled](docker-images.html#pull-image) or [built](docker-images.html#build-image) previously.

![Docker Image run configuration dialog](https://resources.jetbrains.com/help/img/idea/2025.3/docker-image-run-config.png)

Docker uses the [docker run](https://docs.docker.com/engine/reference/commandline/run/) command with the following syntax:
...
Pull image| Define if you want IntelliJ IDEA to pull a Docker image before running the container:

  * Missing: IntelliJ IDEA will pull a Docker image from the registry if it is missing locally.
  * Always: IntelliJ IDEA will always pull a Docker image from the registry.
  * Never: IntelliJ IDEA will never pull a Docker image from the registry.
...
[Docker run configurations](docker-run-configurations.html)[Dockerfile run configuration](dockerfile-run-configuration.html)
